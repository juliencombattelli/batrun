...
impl Executor for RoundRobinExecutor {
    fn execute(&self, execution_contexts: &[ExecutionContext]) {
        ...
        while !contexts.is_empty() {
            ...
            let target = &context.execution_context.target;
            let test_suite_dir = context.execution_context.test_suite.path();
            let test_driver = context.execution_context.test_driver;
            let (done, _) =
                context
                    .visitor
